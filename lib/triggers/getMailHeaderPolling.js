  ...
  async function handleResult(data, last) {
    try {
      if (data === false) {
        ...
        if (!cfg.sendDuplicates && (cleanEmail in usedEmails)) {
          console.log('Email already seen in this fetch -> skipping', cleanEmail);
          return false;
        }

        usedEmails[cleanEmail] = 1;
        ...
        */
        const oihMeta = {
          applicationUid: applicationUid || 'imap',
        };
